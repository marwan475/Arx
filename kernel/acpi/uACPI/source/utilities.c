...
{
    return is_valid_name_byte(nameseg[0]) && is_valid_name_byte(nameseg[1]) && is_valid_name_byte(nameseg[2]) && is_valid_name_byte(nameseg[3]);
}
...
uacpi_status uacpi_eval_cid(uacpi_namespace_node* node, uacpi_pnp_id_list** out_list)
{
    ...
    ret = uacpi_eval_typed(node, "_CID", UACPI_NULL, UACPI_OBJECT_INTEGER_BIT | UACPI_OBJECT_STRING_BIT | UACPI_OBJECT_PACKAGE_BIT, &cid_ret);
    if (ret != UACPI_STATUS_OK)
        return ret;
...
uacpi_status uacpi_find_devices_at(uacpi_namespace_node* parent, const uacpi_char* const* hids, uacpi_iteration_callback cb, void* user)
{
    ...
    return uacpi_namespace_for_each_child(parent, find_one_device, UACPI_NULL, UACPI_OBJECT_DEVICE_BIT, UACPI_MAX_DEPTH_ANY, &ctx);
}
