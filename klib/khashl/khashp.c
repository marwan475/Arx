...
static inline void *khp_malloc(size_t size)
{
	if (size == 0) return 0;
	return vmalloc(size);
}

static inline void *khp_calloc(size_t cnt, size_t elem_size)
{
	size_t size;
	void *p;

	if (cnt == 0 || elem_size == 0) return 0;
	if (cnt > SIZE_MAX / elem_size) return 0;
...
static inline void khp_free(void *ptr)
{
	if (ptr) vfree(ptr);
}

static inline void *khp_realloc(void *ptr, size_t old_size, size_t new_size)
{
	void *new_ptr;
	size_t copy_size;
...
static int khp_strcmp(const char *a, const char *b)
{
	while (*a && *a == *b) {
		++a;
		++b;
...
#define MALLOC(type, cnt) ((type*)khp_malloc((cnt) * sizeof(type)))
#define CALLOC(type, cnt) ((type*)khp_calloc((cnt), sizeof(type)))
...
KHP_SCOPE int khp_resize(khashp_t *h, khint_t new_n_buckets)
{
	...
	if (n_buckets < new_n_buckets) { /* expand */
		size_t old_bucket_bytes = (size_t)n_buckets * (h->key_len + h->val_len);
		size_t new_bucket_bytes = (size_t)new_n_buckets * (h->key_len + h->val_len);
		uint8_t *new_b = (uint8_t*)khp_realloc(h->b, old_bucket_bytes, new_bucket_bytes);
		if (!new_b) { khp_free(new_used); return -1; }
		h->b = new_b;
	} /* otherwise shrink */
	...
	if (n_buckets > new_n_buckets) /* shrink the hash table */
		h->b = (uint8_t*)khp_realloc(h->b,
			(size_t)n_buckets * (h->key_len + h->val_len),
			(size_t)new_n_buckets * (h->key_len + h->val_len));
	khp_free(h->used); /* free the working space */
