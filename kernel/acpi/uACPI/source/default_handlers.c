...
static uacpi_status pci_region_attach(uacpi_region_attach_data* data)
{
    ...
    uacpi_trace("detected PCI device %.4s@%04X:%02X:%02X:%01X\n", device->name.text, address.segment, address.bus, address.device, address.function);
...
static uacpi_status pci_region_do_rw(uacpi_region_op op, uacpi_region_rw_data* data)
{
    ...
    if (!ctx->handle_is_valid)
    {
        uacpi_trace("faking a PCI device %s access\n", op == UACPI_REGION_OP_READ ? "read" : "write");
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_pci_read(ctx->device_handle, offset, width, &data->value) : uacpi_pci_write(ctx->device_handle, offset, width, data->value);
}
...
void uacpi_install_default_address_space_handlers(void)
{
    ...
    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_SYSTEM_MEMORY, handle_memory_region, UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_SYSTEM_IO, handle_io_region, UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_PCI_CONFIG, handle_pci_region, UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_TABLE_DATA, handle_table_data_region, UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);
}
