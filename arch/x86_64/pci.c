...
static void pci_fill_bars(const pci_ecam_region_t* region, uint8_t bus, uint8_t device, uint8_t function, uint8_t header_type, pci_device_t* out_device)
{
    uint8_t bar_regs;
    uint8_t bar;
...
static bool pci_scan_function(const pci_ecam_region_t* region, uint8_t bus, uint8_t dev, uint8_t fn, pci_device_t* devices, size_t* inout_device_count)
{
