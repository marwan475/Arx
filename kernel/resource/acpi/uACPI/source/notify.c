...
uacpi_status uacpi_notify_all(uacpi_namespace_node* node, uacpi_u64 value)
{
    ...
    node_object = uacpi_namespace_node_get_object_typed(node, UACPI_OBJECT_DEVICE_BIT | UACPI_OBJECT_THERMAL_ZONE_BIT | UACPI_OBJECT_PROCESSOR_BIT);
    ...
    if (node_object->handlers->notify_head == UACPI_NULL && g_uacpi_rt_ctx.root_object->handlers->notify_head == UACPI_NULL)
    {
...
uacpi_status uacpi_install_notify_handler(uacpi_namespace_node* node, uacpi_notify_handler handler, uacpi_handle handler_context)
{
    ...
    {
        ret = uacpi_namespace_node_acquire_object_typed(node, UACPI_OBJECT_DEVICE_BIT | UACPI_OBJECT_THERMAL_ZONE_BIT | UACPI_OBJECT_PROCESSOR_BIT, &obj);
        if (uacpi_unlikely_error(ret))
            return ret;
    }
