...
out_bad_cast:
    uacpi_error("attempted to perform an invalid implicit cast (%s -> %s)\n", uacpi_object_type_to_string(src->type), uacpi_object_type_to_string(dst->type));
    return ret;
}
...
static uacpi_status build_table_id(const uacpi_char* opcode, struct uacpi_table_identifiers* out_id, uacpi_buffer* signature, uacpi_buffer* oem_id, uacpi_buffer* oem_table_id)
{
    ...
    uacpi_memcpy_zerout(out_id->oem_table_id, oem_table_id->text, sizeof(out_id->oem_table_id), oem_table_id->size ? oem_table_id->size - 1 : 0);
...
static uacpi_status handle_create_data_region(struct execution_context* ctx)
{
    ...
    ret = build_table_id("DataTableRegion", &table_id, item_array_at(items, 1)->obj->buffer, item_array_at(items, 2)->obj->buffer, item_array_at(items, 3)->obj->buffer);
    if (uacpi_unlikely_error(ret))
        return ret;
...
static uacpi_status handle_load_table(struct execution_context* ctx)
{
    ...
    ret = build_table_id("LoadTable", &table_id, item_array_at(items, 5)->obj->buffer, item_array_at(items, 6)->obj->buffer, item_array_at(items, 7)->obj->buffer);
    if (uacpi_unlikely_error(ret))
        return ret;
    ...
    if (root_path->size > 1)
    {
        ret = uacpi_namespace_node_resolve(ctx->cur_frame->cur_scope, root_path->text, UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_YES, UACPI_PERMANENT_ONLY_NO, &root_node);
        if (uacpi_unlikely_error(ret))
        {
    ...
    if (param_path->size > 1)
    {
        ...
        ret = uacpi_namespace_node_resolve(root_node, param_path->text, UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_YES, UACPI_PERMANENT_ONLY_NO, &param_node);
        if (uacpi_unlikely_error(ret))
        {
    ...
    ret    = prepare_table_load(table.hdr, UACPI_TABLE_LOAD_CAUSE_LOAD_TABLE_OP, method);
    if (uacpi_unlikely_error(ret))
    {
...
static uacpi_status handle_create_field(struct execution_context* ctx)
{
    ...
    while (i < item_array_size(&op_ctx->items))
    {
        ...
        if (item->type == ITEM_NAMESPACE_NODE)
        {
            ...
            field->byte_offset = UACPI_ALIGN_DOWN(bit_offset / 8, field->access_width_bytes, uacpi_size);
...
static void debug_store_no_recurse(const uacpi_char* prefix, uacpi_object* src)
{
    switch (src->type)
    {
        ...
        case UACPI_OBJECT_PROCESSOR:
            uacpi_trace("%s Processor[%d] 0x%08X (%d)\n", prefix, src->processor->id, src->processor->block_address, src->processor->block_length);
            break;
        case UACPI_OBJECT_BUFFER_INDEX:
            uacpi_trace("%s Buffer Index %p[%zu] => 0x%02X\n", prefix, src->buffer_index.buffer->data, src->buffer_index.idx, *buffer_index_cursor(&src->buffer_index));
            break;
...
{
    if (parent->flags == UACPI_REFERENCE_KIND_NAMED && uacpi_object_is(parent->inner_object, UACPI_OBJECT_OPERATION_REGION))
    {
        /*
...
static uacpi_status handle_concatenate_res(struct execution_context* ctx)
{
     ...
     */
    dst_buf[dst_size - 2] = (ACPI_RESOURCE_END_TAG << ACPI_SMALL_ITEM_NAME_IDX) | (sizeof(struct acpi_resource_end_tag) - 1);
...
static uacpi_status handle_deref_of(struct execution_context* ctx)
{
    ...
    is_field    = uacpi_object_is_one_of(unwound_src, UACPI_OBJECT_BUFFER_FIELD_BIT | UACPI_OBJECT_FIELD_UNIT_BIT);
    ...
    if (is_field)
    {
        ...
        if (uacpi_unlikely_error(ret))
        {
            uacpi_error("unable to perform a read from field %p: "
    ...
    return uacpi_object_assign(dst, unwound_src, UACPI_ASSIGN_BEHAVIOR_SHALLOW_COPY);
}
...
#define EXEC_OP_DO_LVL(lvl, reason, ...) uacpi_##lvl("Op 0x%04X ('%s'): " reason "\n", op_ctx->op->code, op_ctx->op->name __VA_ARGS__)
...
#define EXEC_OP_ERR_3(reason, arg0, arg1, arg2) EXEC_OP_DO_ERR(reason, , arg0, arg1, arg2)
...
static uacpi_status typecheck_computational_data(const struct op_context* op_ctx, const uacpi_object* obj)
{
    switch (obj->type)
    {
        ...
        case UACPI_OBJECT_INTEGER:
            return UACPI_STATUS_OK;
        default:
            EXEC_OP_ERR_2("invalid argument type: %s, expected a %s", uacpi_object_type_to_string(obj->type), SPEC_COMPUTATIONAL_DATA);
            return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
    }
}
...
static void trace_named_object_lookup_or_creation_failure(struct call_frame* frame, uacpi_size offset, enum uacpi_parse_op op, uacpi_status ret, enum uacpi_log_level level)
{
    ...
    {
        uacpi_log_lvl(level, "unable to %s named object '%s%s%s': %s\n", action, prefix_path, middle_part, requested_path_to_print, uacpi_status_to_string(ret));
    }
    ...
    for (;;)
    {
        ...
        switch (op)
        {
            ...
            case UACPI_PARSE_OP_TYPECHECK:
            {
                ...
                {
                    EXEC_OP_ERR_2("bad object type: expected %s, got %s!", uacpi_object_type_to_string(expected_type), uacpi_object_type_to_string(item->obj->type));
                    ret = UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
                }
            ...
            case UACPI_PARSE_OP_TYPECHECK_ONE_OF:
            {
                ...
                EXEC_OP_ERR_2("bad object type: expected one of 0x%08X, got %s!", expected_mask, uacpi_object_type_to_string(item->obj->type));
                break;
            }
            ...
            case UACPI_PARSE_OP_EXISTING_NAMESTRING_OR_NULL_IF_LOAD:
            {
                ...
                if (uacpi_unlikely_error(ret))
                {
                    ...
                    if (frame->method->named_objects_persist && (ret == UACPI_STATUS_AML_OBJECT_ALREADY_EXISTS || ret == UACPI_STATUS_NOT_FOUND))
                    {
...
static void execution_context_release(struct execution_context* ctx)
{
    ...
    {
        held_mutexes_array_remove_and_release(&ctx->held_mutexes, *held_mutexes_array_last(&ctx->held_mutexes), FORCE_RELEASE_YES);
    }
...
uacpi_status uacpi_execute_control_method(uacpi_namespace_node* scope, uacpi_control_method* method, const uacpi_object_array* args, uacpi_object** out_obj)
{
    ...
    handle_method_abort:
        uacpi_error("aborting %s due to previous error: %s\n", ctx->cur_frame->method->named_objects_persist ? "table load" : "method invocation", uacpi_status_to_string(ret));
        stack_unwind(ctx);
