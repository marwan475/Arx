...
static void async_run_gpe_handler(uacpi_handle opaque)
{
    ...
    switch (event->handler_type)
    {
        case GPE_HANDLER_TYPE_AML_HANDLER:
        {
            ...
            if (uacpi_unlikely_error(ret))
            {
                uacpi_error("error while executing GPE(%02X) handler %.4s: %s\n", event->idx, event->aml_handler->name.text, uacpi_status_to_string(ret));
            }
            break;
        }
...
static uacpi_status create_gpe_block(uacpi_namespace_node* device_node, uacpi_u32 irq, uacpi_u16 base_idx, uacpi_u64 address, uacpi_u8 address_space_id, uacpi_u16 num_registers)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_match_gpe_methods, UACPI_NULL, UACPI_OBJECT_METHOD_BIT, UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_YES, UACPI_PERMANENT_ONLY_YES, &match_ctx);

    uacpi_trace("initialized GPE block %.4s[%d->%d], %d AML handlers (IRQ %d)\n", device_node->name.text, base_idx, base_idx + block->num_events, match_ctx.matched_count, irq);
    return UACPI_STATUS_OK;
...
static uacpi_iteration_decision do_initialize_gpe_block(struct gpe_block* block, uacpi_handle opaque)
{
    ...
    if (count_enabled)
    {
        uacpi_info("enabled %zu GPEs in block %.4s@[%d->%d]\n", count_enabled, block->device_node->name.text, block->base_idx, block->base_idx + block->num_events);
    }
    return UACPI_ITERATION_DECISION_CONTINUE;
}
...
static uacpi_status do_install_gpe_handler(uacpi_namespace_node* gpe_device, uacpi_u16 idx, uacpi_gpe_triggering triggering, enum gpe_handler_type type, uacpi_gpe_handler handler, uacpi_handle ctx)
{
    ...
    if (event->handler_type == GPE_HANDLER_TYPE_NATIVE_HANDLER || event->handler_type == GPE_HANDLER_TYPE_NATIVE_HANDLER_RAW)
    {
    ...
    if ((event->handler_type == GPE_HANDLER_TYPE_AML_HANDLER || event->handler_type == GPE_HANDLER_TYPE_IMPLICIT_NOTIFY) && event->num_users != 0)
    {
        ...
        if (uacpi_unlikely(event->triggering != triggering))
        {
            uacpi_warn("GPE(%02X) user handler claims %s triggering, originally "
                       "configured as %s\n",
                       idx, uacpi_gpe_triggering_to_string(triggering), uacpi_gpe_triggering_to_string(event->triggering));
        }
    }
...
uacpi_status uacpi_uninstall_gpe_handler(uacpi_namespace_node* gpe_device, uacpi_u16 idx, uacpi_gpe_handler handler)
{
    ...
    if ((event->handler_type == GPE_HANDLER_TYPE_AML_HANDLER || event->handler_type == GPE_HANDLER_TYPE_IMPLICIT_NOTIFY) && native_handler->previously_enabled)
    {
...
static uacpi_status initialize_gpes(void)
{
    ...
    if (fadt->x_gpe0_blk.address && fadt->gpe0_blk_len)
    {
        ...
        ret = create_gpe_block(gpe_node, fadt->sci_int, 0, fadt->x_gpe0_blk.address, fadt->x_gpe0_blk.address_space_id, gpe0_regs);
        if (uacpi_unlikely_error(ret))
        {
    ...
    if (fadt->x_gpe1_blk.address && fadt->gpe1_blk_len)
    {
        ...
        ret = create_gpe_block(gpe_node, fadt->sci_int, fadt->gpe1_base, fadt->x_gpe1_blk.address, fadt->x_gpe1_blk.address_space_id, gpe1_regs);
        if (uacpi_unlikely_error(ret))
        {
...
uacpi_status uacpi_initialize_events(void)
{
    ...
    ret = uacpi_kernel_install_interrupt_handler(g_uacpi_rt_ctx.fadt.sci_int, handle_sci, g_gpe_interrupt_head, &g_uacpi_rt_ctx.sci_handle);
    if (uacpi_unlikely_error(ret))
    {
