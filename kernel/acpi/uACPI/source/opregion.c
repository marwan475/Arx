...
{
    if (space == UACPI_ADDRESS_SPACE_SYSTEM_MEMORY || space == UACPI_ADDRESS_SPACE_SYSTEM_IO || space == UACPI_ADDRESS_SPACE_TABLE_DATA)
        return UACPI_FALSE;
...
static uacpi_status region_run_reg(uacpi_namespace_node* node, uacpi_u8 connection_code)
{
    ...
    ret = uacpi_namespace_node_resolve(node->parent, "_REG", UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_NO, UACPI_PERMANENT_ONLY_NO, &reg_node);
...
static uacpi_status reg_or_unreg_all_opregions(uacpi_namespace_node* device_node, enum uacpi_address_space space, uacpi_u8 connection_code)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_run_reg, UACPI_NULL, UACPI_OBJECT_OPERATION_REGION_BIT, UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, perm_only, &ctx);

    uacpi_trace("%sactivated all '%s' opregions controlled by '%.4s', "
                "%zu _REG() calls (%zu errors)\n",
                connection_code == ACPI_REG_CONNECT ? "" : "de", uacpi_address_space_to_string(space), device_node->name.text, ctx.reg_executed, ctx.reg_errors);
    return UACPI_STATUS_OK;
}
...
static uacpi_address_space_handlers* extract_handlers(uacpi_namespace_node* node)
{
    ...
    handlers_obj = uacpi_namespace_node_get_object_typed(node, UACPI_OBJECT_DEVICE_BIT | UACPI_OBJECT_THERMAL_ZONE_BIT | UACPI_OBJECT_PROCESSOR_BIT);
                                                            ...
                                                            uacpi_u16 flags)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_install_or_uninstall_handler, UACPI_NULL, UACPI_OBJECT_ANY_BIT, UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, UACPI_PERMANENT_ONLY_YES, &iter_ctx);
...
uacpi_status uacpi_uninstall_address_space_handler(uacpi_namespace_node* device_node, enum uacpi_address_space space)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_install_or_uninstall_handler, UACPI_NULL, UACPI_OBJECT_ANY_BIT, UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, UACPI_PERMANENT_ONLY_NO, &iter_ctx);
...
uacpi_status uacpi_dispatch_opregion_io(uacpi_field_unit* field, uacpi_u32 offset, uacpi_region_op op, union uacpi_opregion_io_data data)
{
    ...
    if (uacpi_unlikely(is_oob))
    {
        ...
        uacpi_error("out-of-bounds access to opregion %s[0x%" UACPI_PRIX64 "->"
                    ...
                    path, UACPI_FMT64(region->offset), UACPI_FMT64(region->offset + region->length), UACPI_FMT64(abs_offset), offset, field->access_width_bytes);
        uacpi_free_dynamic_string(path);
