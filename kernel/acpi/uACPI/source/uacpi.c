...
static uacpi_bool match_ssdt_or_psdt(struct uacpi_installed_table* tbl)
{
    ...
    return uacpi_signatures_match(tbl->hdr.signature, ACPI_SSDT_SIGNATURE) || uacpi_signatures_match(tbl->hdr.signature, ACPI_PSDT_SIGNATURE);
}
...
uacpi_status uacpi_namespace_load(void)
{
    ...
    {
        uacpi_info("loaded %u AML blob%s (%u error%s)\n", st.load_counter, st.load_counter > 1 ? "s" : "", st.failure_counter, st.failure_counter == 1 ? "" : "s");
    }
    else
    {
        ...
        uacpi_info("successfully loaded %u AML blob%s, %" UACPI_PRIu64 " ops in "
                   ...
                   st.load_counter, st.load_counter > 1 ? "s" : "", UACPI_FMT64(ops), UACPI_FMT64(elapsed_ms(begin_ts, end_ts)), UACPI_FMT64(ops_per_sec));
    }
...
uacpi_status uacpi_namespace_initialize(void)
{
    ...
    uacpi_trace("_STA calls: %zu (%zu errors), _INI calls: %zu (%zu errors)\n", ctx.sta_executed, ctx.sta_errors, ctx.ini_executed, ctx.ini_errors);
...
uacpi_status uacpi_eval(uacpi_namespace_node* parent, const uacpi_char* path, const uacpi_object_array* args, uacpi_object** out_obj)
{
    ...
    if (path != UACPI_NULL)
    {
        ret = uacpi_namespace_node_resolve(parent, path, UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_NO, UACPI_PERMANENT_ONLY_YES, &node);
        if (uacpi_unlikely_error(ret))
