...
static void dump_table_header(uacpi_phys_addr phys_addr, void* hdr)
{
    ...
    if (!uacpi_memcmp(hdr, ACPI_RSDP_SIGNATURE, sizeof(ACPI_RSDP_SIGNATURE) - 1))
    {
        ...
        uacpi_info("RSDP 0x%016" UACPI_PRIX64 " %08X v%02X (%6.6s)\n", UACPI_FMT64(phys_addr), rsdp->revision >= 2 ? rsdp->length : 20, rsdp->revision, rsdp->oemid);
        return;
    }

    uacpi_info("%.4s 0x%016" UACPI_PRIX64 " %08X v%02X (%6.6s %8.8s)\n", sdt->signature, UACPI_FMT64(phys_addr), sdt->length, sdt->revision, sdt->oemid, sdt->oem_table_id);
}
...
uacpi_status uacpi_setup_early_table_access(void* temporary_buffer, uacpi_size buffer_size)
{
    ...
    aligned_buffer_addr = UACPI_ALIGN_UP(buffer_addr, UACPI_POINTER_SIZE, uacpi_virt_addr);
    if (buffer_addr != aligned_buffer_addr)
    {
        uacpi_size stripped_bytes;

        stripped_bytes = aligned_buffer_addr - buffer_addr;
        uacpi_warn("fixed up misaligned early tables buffer (%zu bytes stripped)\n", stripped_bytes);
...
uacpi_status uacpi_initialize_tables(void)
{
    ...
    if (!uacpi_is_hardware_reduced())
    {
        ...
        if (fadt->x_firmware_ctrl)
        {
            ...
            ret = table_install_physical_with_origin_unlocked(fadt->x_firmware_ctrl, UACPI_TABLE_ORIGIN_FIRMWARE_PHYSICAL, ACPI_FACS_SIGNATURE, &tbl);
            if (uacpi_unlikely(ret != UACPI_STATUS_OK && ret != UACPI_STATUS_OVERRIDDEN))
                return ret;
...
static uacpi_status handle_table_override(uacpi_table_installation_disposition disposition, uacpi_u64 address, uacpi_table* out_table)
{
    ...
    switch (disposition)
    {
        case UACPI_TABLE_INSTALLATION_DISPOSITON_VIRTUAL_OVERRIDE:
            ret = table_install_with_origin_unlocked(UACPI_VIRT_ADDR_TO_PTR((uacpi_virt_addr) address), UACPI_TABLE_ORIGIN_HOST_VIRTUAL, out_table);
            return ret;
        case UACPI_TABLE_INSTALLATION_DISPOSITON_PHYSICAL_OVERRIDE:
            return table_install_physical_with_origin_unlocked((uacpi_phys_addr) address, UACPI_TABLE_ORIGIN_HOST_PHYSICAL, UACPI_NULL, out_table);
        default:
...
static uacpi_status table_install_physical_with_origin_unlocked(uacpi_phys_addr phys, enum uacpi_table_origin origin, const uacpi_char* expected_signature, uacpi_table* out_table)
{
    ...
    {
        uacpi_warn("bogus table '%.4s' (0x%016" UACPI_PRIX64 ") size: %u bytes\n", hdr.signature, UACPI_FMT64(phys), hdr.length);
    }
    ...
    if (origin == UACPI_TABLE_ORIGIN_FIRMWARE_PHYSICAL && installation_handler != UACPI_NULL)
    {
        ...
        switch (disposition)
        {
            ...
            case UACPI_TABLE_INSTALLATION_DISPOSITON_DENY:
                uacpi_info("table '%.4s' (0x%016" UACPI_PRIX64 ") installation denied "
                           "by host\n",
...
static uacpi_iteration_decision do_search_tables(void* user, struct uacpi_installed_table* tbl, uacpi_size idx)
{
    ...
    switch (ctx->search_type)
    {
        case SEARCH_TYPE_BY_ID:
        {
            ...
            if (id->oem_table_id[0] != '\0' && uacpi_memcmp(id->oem_table_id, tbl->hdr.oem_table_id, sizeof(id->oem_table_id)) != 0)
                return UACPI_ITERATION_DECISION_CONTINUE;
...
static uacpi_status initialize_fadt(const void* virt)
{
    ...
    if (fadt->x_dsdt)
    {
        ret = table_install_physical_with_origin_unlocked(fadt->x_dsdt, UACPI_TABLE_ORIGIN_FIRMWARE_PHYSICAL, ACPI_DSDT_SIGNATURE, UACPI_NULL);
        if (uacpi_unlikely(ret != UACPI_STATUS_OK && ret != UACPI_STATUS_OVERRIDDEN))
            return ret;
    }
...
uacpi_status uacpi_for_each_subtable(struct acpi_sdt_hdr* hdr, size_t hdr_size, uacpi_subtable_iteration_callback cb, void* user)
{
    ...
    while (bytes_left > sizeof(struct acpi_entry_hdr))
    {
        ...
        if (uacpi_unlikely(subtable_hdr->length > bytes_left || subtable_hdr->length < sizeof(*subtable_hdr)))
        {
            uacpi_error("corrupted '%.4s' subtable length: %u (%zu bytes left)\n", hdr->signature, subtable_hdr->length, bytes_left);
            return UACPI_STATUS_INVALID_TABLE_LENGTH;
        }
