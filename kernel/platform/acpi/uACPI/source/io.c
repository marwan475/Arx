...
static uacpi_u8 gas_get_access_bit_width(const struct acpi_gas* gas)
{
    ...
    if (gas->register_bit_offset == 0 && UACPI_IS_POWER_OF_TWO(gas->register_bit_width, uacpi_u8) && UACPI_IS_ALIGNED(gas->register_bit_width, 8, uacpi_u8))
    {
...
static uacpi_status gas_validate(const struct acpi_gas* gas, uacpi_u8* access_bit_width, uacpi_u8* bit_width)
{
    ...
    if (gas->address_space_id != UACPI_ADDRESS_SPACE_SYSTEM_IO && gas->address_space_id != UACPI_ADDRESS_SPACE_SYSTEM_MEMORY)
    {
        uacpi_warn("unsupported GAS address space '%s' (%d)\n", uacpi_address_space_to_string(gas->address_space_id), gas->address_space_id);
        return UACPI_STATUS_UNIMPLEMENTED;
    }
